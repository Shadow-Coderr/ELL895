from .utils import mul2

# S-box in 16x16 matrix form
# fmt: off
sBox = [
...
def subBytes(state):
    """
    Substitute the bytes of the state matrix using the S-box.
...
def mixColumns(state):
    ...
    for i in range(4):
        ...
        tmp = a ^ b ^ c ^ d
        state[0][i] = a ^ tmp ^ mul2(a ^ b)
        state[1][i] = b ^ tmp ^ mul2(b ^ c)
        state[2][i] = c ^ tmp ^ mul2(c ^ d)
        state[3][i] = d ^ tmp ^ mul2(d ^ a)
...
def keyExpansion(key):
    ...
    for i in range(4, 44):
        ...
        if i % 4 == 0:
            ...
            t2 = sBox[temp >> 28][temp >> 24 & 0x0F] << 24
            t2 |= sBox[temp >> 20 & 0x0F][temp >> 16 & 0x0F] << 16
            t2 |= sBox[temp >> 12 & 0x0F][temp >> 8 & 0x0F] << 8
            t2 |= sBox[temp >> 4 & 0x0F][temp & 0x0F]
